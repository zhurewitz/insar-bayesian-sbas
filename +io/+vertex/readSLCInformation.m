...
function metaData= readSLCInformation(JSONfilename)
...
for j= 1:N
    metaData.Polyshape(j)= polyshape(x(j,:),y(j,:));
    [~,cy]= centroid(metaData.Polyshape(j));
...
metaData= metaData(I,:);
metaData= sortrows(metaData,"Date");
